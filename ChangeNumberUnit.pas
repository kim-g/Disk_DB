...
with Query.SQL do
  begin
  Clear;
  Add('INSERT INTO [Члены труппы] ([Табельный номер], Фамилия, Имя, Отчество, Действующий, Пол)');
  Add('VALUES (' + Edit1.Text + ', "' + InBase.FieldByName('Фамилия').AsString + '", "' + InBase.FieldByName('Имя').AsString +
    '", "' + InBase.FieldByName('Отчество').AsString + '", ' + InBase.FieldByName('Действующий').AsString + ', ' +
    InBase.FieldByName('Пол').AsString+');');
  end;
...
with Query.SQL do
  begin
  Clear;
  Add('UPDATE Список_дисков');
  Add('SET Взят = ' + Edit1.Text);
  Add('WHERE Взят = ' + IntToStr(NumberToChange));
  end;
Query.ExecSQL;
...
Label2.Caption:='Введите табельный номер.';
Label2.Visible:=true;
...
with Query.SQL do
  begin
  Clear;
  Add('SELECT Count(*) AS NRec FROM [Члены труппы] WHERE [Табельный номер] = ' + Edit1.Text + ';');
  end;
Query.Active:=true;
...
if N_Records > 0 then
  ...
  Button1.Enabled:=false;
  Image1.Picture.LoadFromFile('NO.png');
  Label2.Caption:='Табельный номер уже используется.';
  Label2.Visible:=true;
  exit;
  end;
