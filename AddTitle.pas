...
if List.Items.IndexOf(Edit1.Text)=-1 then
  ...
  with Query do
    begin
    SQL.Clear;
    SQL.Add('INSERT INTO Сюжеты (Название)');
...
if List.ItemIndex=-1 then
  begin
  ShowMessage('Выберите название в списке.');
  exit;
  end;
...
with Query do
    begin
    SQL.Clear;
    SQL.Add('SELECT ID_Сюжета, Название');
    SQL.Add('FROM Сюжеты');
    if Edit1.Text<>'' then SQL.Add('WHERE Название LIKE ''%' + Edit1.Text+'%'' ');
    SQL.Add('ORDER BY Название;');
    open;
    while not Eof do
      begin
        List.Items.Add(FieldByName('Название').AsString );
        ID.Add(FieldByName('ID_Сюжета').AsString);
        next;
      end;
    close;
    end;
