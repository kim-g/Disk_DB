...
with Query do
  begin
  SQL.Clear;
  SQL.Add('SELECT MAX([Номер в каталоге]) AS NDisk');
  ...
  while not Eof do
    begin
    Got.Items.Add(FieldByName('Персона').AsString);
    ID_Got.Add(FieldByName('Табельный номер').AsString);
    next;
    end;
  close;
  end;
...
with Query do
  ...
  SQL.Add('FROM Список_дисков');
  SQL.Add('WHERE [Номер в каталоге]='+NDisk.Text+';');
  Open;
  if FieldByName('NDisk').AsInteger > 0 then
    begin
    ShowMessage('Ошибка!'#13#10'Такой номер диска уже есть! Пожалуйста, выберите предложенный или больший.');
    exit;
    end;
  Close;
  SQL.Clear;
  SQL.Add('INSERT INTO Список_Дисков ([Номер в каталоге], Видеозапись, Взят, Дата_Взятия)');
