...
NewDisk.Video:=StrToInt(ID[List.ItemIndex]);
NewDisk.Video_Text:=List.Items[List.ItemIndex];
Cont:=true;
close;
end;
...
with Query do
  ...
  SQL.Add(' Видеозаписи.Дата_Спектакля, МестоНаПолке.[Место на полке], Видеозаписи.Комментарий');
  SQL.Add('FROM МестоНаПолке ');
